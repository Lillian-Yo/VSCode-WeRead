...
exports.getTemplateService = exports.TemplateService = void 0;
...
`;
    ...
    getCustomTemplate() {
        ...
        const customTemplate = config.get('noteTemplate');
    ...
    validateTemplate(template) {
        ...
        try {
            ...
            this.renderTemplateString(template, mock);
            return { valid: true };
        }
        catch (error) {
            return {
     ...
     * 构建模板数据
     */
    buildTemplateData(book, notes, review) {
        // 按章节分组笔记
     ...
     * 简单格式渲染（备用）
     */
    renderSimple(book, notes, review) {
        const lines = [
            `# ${book.title}`,
            '',
    ...
    renderFileName(book) {
        const config = vscode.workspace.getConfiguration('weread');
        const template = config.get('fileNameTemplate', '{{title}}');
        const data = {
            title: book.title,
            author: book.author,
            isbn: book.isbn || '',
    ...
    buildChaptersMarkdown(chapters) {
    ...
    buildBookInfoMarkdown(book) {
        const lines = this.buildBookInfoLines(book);
        return lines.length > 0 ? lines.join('\n') : '暂无可展示的书籍信息';
    }
    buildBookInfoLines(book) {
        const rawLines = [
            ['**作者**', book.author],
            ['**doc_type**', book.docType],
            ['**bookid**', book.rawBookId || book.bookId],
...
exports.getTemplateService = getTemplateService;
