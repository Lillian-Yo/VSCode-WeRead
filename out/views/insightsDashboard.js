...
exports.getInsightsDashboardView = exports.initializeInsightsDashboardView = exports.InsightsDashboardView = void 0;
const vscode = __importStar(require("vscode"));
const analyticsService_1 = require("../services/analyticsService");
const exportService_1 = require("../services/exportService");
const auth_1 = require("../auth");
const accountMetaManager_1 = require("../services/accountMetaManager");
class InsightsDashboardView {
    ...
    async show() {
        ...
        panel.webview.onDidReceiveMessage(async (message) => {
            ...
            if (message?.command === 'exportReport') {
                ...
                const loaded = await (0, analyticsService_1.getAnalyticsService)().getDashboardDataResilient(filter, accountId, {
                    retries: 2,
                    retryDelayMs: 300,
                });
                const data = loaded.data;
                const result = await (0, exportService_1.getExportService)().exportInsightsReport(data);
                ...
                await (0, exportService_1.getExportService)().openExportedFile(result.filePath);
            }
        });
    ...
    normalizeFilter(filter) {
        ...
        const days = payload.days === 0 || String(payload.days) === '0'
            ? 0
    ...
    async pushData(filter) {
        ...
        try {
            const result = await (0, analyticsService_1.getAnalyticsService)().getDashboardDataResilient(filter, this.getActiveAccountId(), {
                retries: 2,
                retryDelayMs: 350,
            });
            ...
            console.error(`[Insights][account:${account.accountId || 'default'}] 面板数据加载失败：${message}`);
            await panel.webview.postMessage({
                command: 'renderData',
                data: (0, analyticsService_1.getAnalyticsService)().getDashboardData(filter, this.getActiveAccountId()),
                account,
            });
            await panel.webview.postMessage({
                command: 'setStatus',
    ...
    getActiveAccountId() {
        const fromCookie = (0, auth_1.getCookieManager)().getActiveAccountId();
        if (fromCookie) {
            return fromCookie;
        }
        const fromMeta = (0, accountMetaManager_1.getAccountMetaManager)().getActiveAccountId();
        if (fromMeta) {
            return fromMeta;
        }
        return (0, accountMetaManager_1.getAccountMetaManager)().listAccounts()[0]?.accountId;
    }
    getActiveAccountPresentation() {
        const accountId = this.getActiveAccountId();
        if (!accountId) {
            return {};
        }
        const profile = (0, accountMetaManager_1.getAccountMetaManager)().listAccounts().find((item) => item.accountId === accountId);
        return {
            accountId,
            displayName: profile?.displayName || accountId,
        };
    }
    renderShellHtml(webview) {
...
exports.InsightsDashboardView = InsightsDashboardView;
function buildNonce() {
...
let insightsDashboardViewInstance;
function initializeInsightsDashboardView(extensionUri) {
    insightsDashboardViewInstance = new InsightsDashboardView(extensionUri);
    return insightsDashboardViewInstance;
}
exports.initializeInsightsDashboardView = initializeInsightsDashboardView;
function getInsightsDashboardView() {
    if (!insightsDashboardViewInstance) {
        throw new Error('InsightsDashboardView not initialized');
    }
    return insightsDashboardViewInstance;
}
exports.getInsightsDashboardView = getInsightsDashboardView;
//# sourceMappingURL=insightsDashboard.js.map
