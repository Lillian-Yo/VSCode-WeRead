...
class QRLoginManager {
     ...
     */
    async startProtocolLogin(options) {
     ...
     */
    async startCookieLogin(options) {
        this.dispose();
        this.webviewPanel = vscode.window.createWebviewPanel('wereadCookieLogin', '微信读书 - 粘贴 Cookie 登录', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    isProtocolRunActive(runId) {
        return this.protocolRunId === runId;
    }
    openProtocolLoginPanel(loginUrl, runId) {
        this.dispose();
        this.webviewPanel = vscode.window.createWebviewPanel('wereadProtocolLogin', '微信读书 - 扫码登录', vscode.ViewColumn.One, {
            enableScripts: true,
            retainContextWhenHidden: true,
        });
    ...
    async getLoginUid() {
        const data = await this.fetchJson('https://weread.qq.com/api/auth/getLoginUid');
        const uid = data?.uid;
        if (!uid) {
    ...
    async waitForLoginConfirm(uid, runId) {
        ...
        while (Date.now() < timeoutAt && this.isProtocolRunActive(runId)) {
            ...
            if (result?.succeed && result.accessToken && result.webLoginVid) {
                ...
                const expiresAt = expiresRaw > 0 ? (expiresRaw < 1000000000000 ? expiresRaw * 1000 : expiresRaw) : undefined;
                return {
    ...
    emitProtocolProgress(logicCode) {
        switch (logicCode) {
            case 'SCAN_SUCCESS':
            case 'SCANNED':
    ...
    normalizeCookieValue(input, key) {
    ...
    async getUserProfileByToken(vid, skey) {
     ...
     * 处理登录成功
     */
    async handleLoginSuccess(cookies, vid, skey, authPayload, options) {
        try {
            let userProfile;
            if (vid && skey) {
                try {
            ...
            if (targetAccountId && targetAccountId !== resolvedAccountId) {
                const confirm = await vscode.window.showWarningMessage(`目标账号与实际登录账号不一致（目标: ${targetAccountId}，实际: ${resolvedAccountId}），是否继续写入实际账号？`, { modal: true }, '继续');
                if (confirm !== '继续') {
                    return false;
                }
            }
            ...
            await (0, accountMetaManager_1.getAccountMetaManager)().setActiveAccountId(resolvedAccountId);
     ...
     * 获取用户信息
     */
    async getUserProfile(cookies) {
        // 临时设置 Cookie 获取用户信息
        api_1.apiClient.setCookies(cookies);
        const response = await api_1.apiClient.get('/user/profile');
        return {
...
exports.QRLoginManager = QRLoginManager;
