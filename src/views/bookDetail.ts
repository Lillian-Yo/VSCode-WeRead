...
import { Book, Note, Chapter } from '../models';
import { getStorageService } from '../services/storageService';
import { WebviewManager, initializeWebviewManager, getWebviewManager } from './webviewManager';

export class BookDetailView {
  private webviewManager: WebviewManager;

  constructor(extensionUri: vscode.Uri) {
    this.webviewManager = initializeWebviewManager(extensionUri);
  }
   ...
   * 生成 HTML 内容
   */
  private generateHtml(book: Book, notes: Note[]): string {
...
<body>
   ...
   * 按章节分组笔记
   */
  private groupNotesByChapter(notes: Note[]): Map<string, Note[]> {
   ...
   * 提取章节信息
   */
  private extractChapters(notes: Note[]): Chapter[] {
   ...
   * 渲染章节树
   */
  private renderChapterTree(chapters: Chapter[], groupedNotes: Map<string, Note[]>): string {
   ...
   * 渲染头部
   */
  private renderHeader(book: Book): string {
    return `
   ...
   * 渲染笔记列表
   */
  private renderNotes(groupedNotes: Map<string, Note[]>): string {
...
let bookDetailViewInstance: BookDetailView | undefined;

export function initializeBookDetailView(extensionUri: vscode.Uri): BookDetailView {
  bookDetailViewInstance = new BookDetailView(extensionUri);
  return bookDetailViewInstance;
}

export function getBookDetailView(): BookDetailView {
