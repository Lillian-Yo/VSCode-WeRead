...
import { getNoteRoamingService } from '../services/noteRoamingService';
import { getStorageService } from '../services/storageService';
import { RoamingCandidate, RoamingFilter, RoamingNoteTypeFilter } from '../types/noteRoaming';
import { logNoteRoaming } from '../logging/noteRoamingLog';

type SessionState = {
  accountId?: string;
  accountName?: string;
  pool: RoamingCandidate[];
  currentIndex: number;
  history: RoamingCandidate[];
  filter: RoamingFilter;
  metrics: {
    reviewed: number;
    openedSource: number;
    skipped: number;
...
function isSameNaturalDay(a: number, b: number): boolean {
...
function normalizeToMs(value: unknown): number {
  const num = Number(value || 0);
  if (!num || !Number.isFinite(num)) {
    return 0;
  }
  return num > 1_000_000_000_000 ? Math.floor(num) : Math.floor(num * 1000);
}

export class NoteRoamingView {
  private static currentPanel: vscode.WebviewPanel | undefined;
  private state!: SessionState;
  private statsPool: RoamingCandidate[] = [];
  private webviewReady = false;
  private webviewReadyWaiters: Array<(ready: boolean) => void> = [];
  ...
  private resetWebviewReadyState(): void {
  ...
  private async waitForWebviewReady(timeoutMs = 5000): Promise<boolean> {
  ...
  private async postToWebview(message: unknown, scene: string): Promise<boolean> {
    ...
    try {
      ...
      if (!ok) {
        logNoteRoaming(`webview postMessage dropped scene=${scene}`, 'WARN');
      }
      return ok;
    } catch (error) {
      const detail = error instanceof Error ? error.message : String(error);
      logNoteRoaming(`webview postMessage failed scene=${scene} error=${detail}`, 'ERROR');
      return false;
    }
  }
  ...
  constructor(private readonly extensionUri: vscode.Uri) {
    ...
    async show() {
        if (NoteRoamingView.currentPanel) {
            ...
            if (!ready) {
                logNoteRoaming('webview ready timeout (reuse panel)', 'WARN');
            }
        ...
        panel.webview.onDidReceiveMessage(async (message) => {
            ...
            if (command === 'applyFilter') {
                ...
                logNoteRoaming(`applyFilter accountId=${this.state.accountId || 'none'} favoriteOnly=${this.state.filter.favoriteOnly ? 1 : 0} noteTypes=${(this.state.filter.noteTypes || []).join(',') || 'all'} minDays=${this.state.filter.minDaysUnreviewed || 0}`);
                await this.reloadPool();
                return;
            }
            if (command === 'next') {
            ...
            if (command === 'webviewError') {
                ...
                logNoteRoaming(`webview runtime error message=${messageText} stack=${stack}`, 'ERROR');
                return;
            }
            if (command === 'webviewLog') {
                const level = String(message?.level || 'INFO').toUpperCase() as 'INFO' | 'WARN' | 'ERROR';
                const stage = String(message?.stage || 'unknown');
                const detail = String(message?.detail || '');
                logNoteRoaming(`webview export stage=${stage} detail=${detail}`, level);
                return;
            }
        });
        ...
        const ready = await this.waitForWebviewReady();
        if (!ready) {
            logNoteRoaming('webview ready timeout (new panel)', 'WARN');
        }
        await this.reloadPool();
    }
    ...
    async reloadPool() {
        ...
        logNoteRoaming(`reloadPool start accountId=${accountId || 'none'}`);
        if (!accountId) {
            logNoteRoaming('reloadPool aborted: no active account', 'WARN');
        ...
        logNoteRoaming(`reloadPool loaded candidates=${pool.length} accountId=${accountId}`);
        const roamingMeta = getStorageService().getRoamingMeta(accountId);
        const favoriteMetaRecords = Object.values(roamingMeta.records || {}).filter((item) => !!item?.favorite);
        const legacyLocalFavoriteRecords = favoriteMetaRecords.filter((item) =>
            /^local_[a-z0-9]+_\d{9,}$/i.test(String(item?.noteKey || ''))
        ).length;
        if (this.state.filter.favoriteOnly) {
            const favoriteRecords = favoriteMetaRecords.length;
            logNoteRoaming(`reloadPool favoriteOnly accountId=${accountId} favoriteRecords=${favoriteRecords} loadedCandidates=${pool.length} minDays=${this.state.filter.minDaysUnreviewed || 0}`);
        }
        this.state = {
            accountId,
            accountName,
        ...
        if (pool.length === 0) {
            ...
            if (this.state.filter.favoriteOnly) {
                logNoteRoaming(`reloadPool favoriteOnly empty accountId=${accountId} minDays=${this.state.filter.minDaysUnreviewed || 0} noteTypes=${(this.state.filter.noteTypes || []).join(',') || 'all'}`, 'WARN');
            ...
            logNoteRoaming(`reloadPool empty accountId=${accountId}`, 'WARN');
            await this.postToWebview({
                command: 'renderEmpty',
                reason: emptyReason,
            }, 'reloadPool:empty');
    ...
    async next() {
        ...
        if (this.state.currentIndex + 1 < this.state.history.length) {
            ...
            logNoteRoaming(`next from history index=${this.state.currentIndex + 1} total=${this.state.history.length}`);
            await this.renderCurrent();
            return;
        }
        const current = this.getCurrent();
        if (current) {
            await getNoteRoamingService().recordAction(current.noteKey, 'skip', this.state.accountId);
            current.meta.skipCount += 1;
            this.state.metrics.skipped += 1;
        }
        const recent = this.state.history.slice(-20);
        const picked = getNoteRoamingService().pickNext(this.state.pool, {
        ...
        if (!picked.candidate) {
            logNoteRoaming('next failed: no candidate', 'WARN');
            await this.postToWebview({
                command: 'renderEmpty',
        ...
        logNoteRoaming(`next picked noteKey=${picked.candidate.noteKey} eligible=${picked.eligibleCandidates}/${picked.totalCandidates} history=${this.state.history.length}`);
        await getNoteRoamingService().recordAction(picked.candidate.noteKey, 'view', this.state.accountId);
        await this.renderCurrent();
    }
    async prev() {
        if (this.state.currentIndex <= 0) {
            logNoteRoaming('prev reached beginning', 'WARN');
            await this.renderCurrent('已经是第一条历史卡片');
            return;
        }
        this.state.currentIndex -= 1;
        logNoteRoaming(`prev index=${this.state.currentIndex + 1}`);
        await this.renderCurrent();
    }
    async toggleFavorite() {
        ...
        await getNoteRoamingService().recordAction(current.noteKey, action, this.state.accountId);
        ...
        logNoteRoaming(`toggleFavorite noteKey=${current.noteKey} before=${oldFavorite ? 1 : 0} after=${current.meta.favorite ? 1 : 0} accountId=${this.state.accountId || 'none'} filterFavoriteOnly=${this.state.filter.favoriteOnly ? 1 : 0}`);
    ...
    async markReviewed() {
        ...
        await getNoteRoamingService().recordAction(current.noteKey, 'review', this.state.accountId);
        current.meta.lastReviewedAt = Date.now();
        current.meta.reviewCount += 1;
        this.state.metrics.reviewed += 1;
        logNoteRoaming(`markReviewed noteKey=${current.noteKey} reviewCount=${current.meta.reviewCount}`);
    ...
    async openSource() {
        ...
        await getNoteRoamingService().recordAction(current.noteKey, 'openSource', this.state.accountId);
        this.state.metrics.openedSource += 1;
        logNoteRoaming(`openSource bookId=${current.bookId} noteKey=${current.noteKey}`);
    ...
    async openEdit() {
        ...
        logNoteRoaming(`openEdit bookId=${current.bookId} noteKey=${current.noteKey}`);
        await vscode.commands.executeCommand('weread.openBookDetail', current.bookId);
        await this.renderCurrent('已打开编辑入口，可直接在文件中修改对应笔记');
    }
    async exportCardImage(imageDataUrl: string, fileNameHint?: string) {
        const panel = NoteRoamingView.currentPanel;
        if (!panel) {
            return;
        }
        logNoteRoaming(`exportCardImage start fileNameHint=${fileNameHint || 'none'} dataUrlLength=${imageDataUrl ? imageDataUrl.length : 0}`);
        if (!/^data:image\/png;base64,/.test(imageDataUrl || '')) {
            logNoteRoaming(`exportCardImage invalidData fileNameHint=${fileNameHint || 'none'} prefix=${String(imageDataUrl || '').slice(0, 32)}`, 'WARN');
        ...
        try {
            ...
            logNoteRoaming(`exportCardImage success path=${saveUri.fsPath}`);
            vscode.window.showInformationMessage(`笔记卡片已导出：${saveUri.fsPath}`);
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            logNoteRoaming(`exportCardImage failed error=${message}`, 'ERROR');
            vscode.window.showErrorMessage(`导出图片失败：${message}`);
        }
    }
    sanitizeExportFileName(raw?: string): string {
    ...
    async renderCurrent(tip?: string) {
        const panel = NoteRoamingView.currentPanel;
        if (!panel) {
            return;
        }
    ...
    normalizeFilter(raw: unknown): RoamingFilter {
        const data = (raw || {}) as Record<string, unknown>;
        const noteTypes = Array.isArray(data.noteTypes)
            ? data.noteTypes.filter((item: unknown) => ['highlight', 'thought', 'chapter', 'review'].includes(String(item)))
            : undefined;
        const minDaysUnreviewed = Number(data.minDaysUnreviewed || 0);
        return {
            noteTypes:
                noteTypes && noteTypes.length > 0
                    ? (noteTypes as RoamingNoteTypeFilter[])
                    : undefined,
    ...
    async loadPoolWithRetry(accountId: string) {
        const service = getNoteRoamingService();
        const retries = 2;
        for (let i = 0; i <= retries; i++) {
            try {
                ...
                const message = error instanceof Error ? error.message : String(error);
                logNoteRoaming(`loadPool failed attempt=${i + 1} accountId=${accountId} error=${message}`, i < retries ? 'WARN' : 'ERROR');
                if (i >= retries) {
                    break;
                }
            }
        ...
        try {
            logNoteRoaming(`loadPool fallback to default account scope accountId=${accountId}`, 'WARN');
    ...
    async loadStatsPool(accountId: string) {
        const service = getNoteRoamingService();
        try {
    ...
    getActiveAccountId() {
        return getCookieManager().getActiveAccountId()
            || getAccountMetaManager().getActiveAccountId()
            || getAccountMetaManager().listAccounts()[0]?.accountId;
    }
    getAccountName(accountId: string | undefined) {
        if (!accountId) {
            return undefined;
        }
        return getAccountMetaManager().listAccounts().find((item) => item.accountId === accountId)?.displayName || accountId;
    }
    renderHtml(webview: vscode.Webview) {
    const configThemes = vscode.workspace.getConfiguration('weread').get('exportThemes') as any[];
    const defaultThemes = [
      { id: 'white', name: '纯白', bg: '#FFFFFF', primary: '#312927', weekday: '#5A514D', divider: '#D5CEC4', watermark: '#757575' },
      { id: 'gray', name: '浅灰', bg: '#F5F5F5', primary: '#312927', weekday: '#5A514D', divider: '#D5CEC4', watermark: '#757575' },
      { id: 'yellow', name: '暖黄', bg: '#FDF6E3', primary: '#382C24', weekday: '#5C4A3D', divider: '#E6D3B8', watermark: '#8C7765' },
      { id: 'green', name: '浅绿', bg: '#EAF4EC', primary: '#1E3624', weekday: '#3D5C45', divider: '#C2DBC7', watermark: '#6F8C76' },
      { id: 'dark', name: '暗夜黑', bg: '#1E1E1E', primary: '#E0E0E0', weekday: '#B0B0B0', divider: '#404040', watermark: '#808080' },
      { id: 'blue', name: '深蓝', bg: '#1A2332', primary: '#DDE2ED', weekday: '#A9B4C7', divider: '#34415B', watermark: '#7887A1' },
      { id: 'pink', name: '莫兰迪粉', bg: '#F6EBEA', primary: '#4A3130', weekday: '#6B4A49', divider: '#E0CACA', watermark: '#9C7A79' },
      { id: 'kraft', name: '牛皮纸', bg: '#E8DCC8', primary: '#3B2F23', weekday: '#594A3A', divider: '#C9B8A1', watermark: '#8A7B68' },
    ];
...
<body>
  ...
  <script nonce="${nonce}">
    ...
    function normalizeExportText(raw) {
      return String(raw || '')
        .replace(/\\r\\n/g, '\\n')
        .replace(/\\r/g, '\\n')
        .trim();
    }
    function wrapNoteLinesByHanCount(text, charsPerLine) {
      ...
      const lines = [];
      const max = Math.max(1, Number(charsPerLine) || 18);
      for (let pi = 0; pi < paras.length; pi++) {
        const chars = Array.from(paras[pi]);
        for (let offset = 0; offset < chars.length; offset += max) {
          lines.push(chars.slice(offset, offset + max).join(''));
        }
      }
      return lines;
    }
    function wrapTitleLines(title, maxChars, maxLines) {
      const t = String(title || '').trim();
      if (!t) {
        return [];
      }
      const chars = Array.from(t);
      const limit = Math.max(1, Number(maxChars) || 22);
      const out = [];
      let i = 0;
      ...
      if (i < chars.length && out.length > 0) {
        ...
        out[out.length - 1] = lastChars.length > 1 ? lastChars.slice(0, lastChars.length - 1).join('') + '…' : '…';
      }
      return out;
    }
    function wrapAuthorLines(author, charsPerLine, maxLines) {
      const t = String(author || '').trim();
      if (!t) {
        return [];
      }
      return wrapTitleLines(t, charsPerLine, maxLines);
    }
    ...
    function generateExportDataUrl() {
      ...
      try {
        ...
        const CARD_W = 400;
        const SCALE = 3; // Use 3x scale for better retina quality
        const PAD_TOP = 60;
        const PAD_BOTTOM = 60;
        const CHARS_PER_LINE = 18;
        
        const DAY_FONT = '800 110px -apple-system, "PingFang SC", "Microsoft YaHei", sans-serif';
        const MONTH_FONT = '700 22px -apple-system, "PingFang SC", "Microsoft YaHei", sans-serif';
        const WEEK_FONT = '400 13px -apple-system, "PingFang SC", "Microsoft YaHei", sans-serif';
        const BODY_FONT = '400 17px -apple-system, "PingFang SC", "Microsoft YaHei", sans-serif';
        const BODY_LINE_HEIGHT = 34;
        const TITLE_FONT = '400 15px -apple-system, "PingFang SC", "Microsoft YaHei", serif';
        const TITLE_LINE_STEP = 24;
        const AUTHOR_FONT = '400 13px -apple-system, "PingFang SC", "Microsoft YaHei", sans-serif';
        const AUTHOR_LINE_STEP = 20;
        const FOOTER_FONT = '400 12px -apple-system, "PingFang SC", "Microsoft YaHei", sans-serif';
        
        const GAP_DAY_MONTH_BASELINE = 12;
        const GAP_MONTH_WEEK = 12;
        const MONTH_LINE_HEIGHT = 28;
        const GAP_WEEK_DIVIDER = 20;
        const GAP_DIVIDER_BODY = 40;
        const GAP_BODY_TITLE = 24;
        const GAP_TITLE_AUTHOR = 8;
        const GAP_AUTHOR_FOOTER = 30;
        const FOOTER_BOTTOM_PAD = 30;
        const DIVIDER_HALF_W = 24;
        ...
        const contentText = String(currentCard.rawText || currentCard.text || currentCard.content || '').trim();
        const bodyLines = wrapNoteLinesByHanCount(contentText, CHARS_PER_LINE);
        const bookTitleRaw = String(currentCard.bookTitle || '未命名书籍').trim();
        const bookTitleDisplay = bookTitleRaw.indexOf('《') === 0 ? bookTitleRaw : ('《' + bookTitleRaw + '》');
        const titleLines = wrapTitleLines(bookTitleDisplay, CHARS_PER_LINE, 3);
        const noteAuthor = resolveDisplayAuthor(currentCard);
        const authorLines = wrapAuthorLines(noteAuthor, CHARS_PER_LINE, 3);
        
        const dayBaseline = PAD_TOP + 100;
        const monthBaseline = dayBaseline + GAP_DAY_MONTH_BASELINE;
        const weekBaseline = monthBaseline + MONTH_LINE_HEIGHT + GAP_MONTH_WEEK;
        const dividerY = weekBaseline + 5 + GAP_WEEK_DIVIDER;
        const bodyFirstBaseline = dividerY + GAP_DIVIDER_BODY;
        var titleFirstBaseline = bodyFirstBaseline;
        if (bodyLines.length > 0) {
          titleFirstBaseline = bodyFirstBaseline + (bodyLines.length - 1) * BODY_LINE_HEIGHT + GAP_BODY_TITLE;
        } else {
          titleFirstBaseline = bodyFirstBaseline + 22;
        }
        var lastTitleBaseline = titleFirstBaseline;
        if (titleLines.length > 0) {
          lastTitleBaseline = titleFirstBaseline + (titleLines.length - 1) * TITLE_LINE_STEP;
        }
        var firstAuthorBaseline = lastTitleBaseline + (titleLines.length > 0 ? GAP_TITLE_AUTHOR : 8);
        if (!titleLines.length && authorLines.length > 0) {
          firstAuthorBaseline = titleFirstBaseline;
        }
        var lastAuthorBaseline = firstAuthorBaseline;
        if (authorLines.length > 0) {
          lastAuthorBaseline = firstAuthorBaseline + (authorLines.length - 1) * AUTHOR_LINE_STEP;
        } else if (titleLines.length > 0) {
          lastAuthorBaseline = lastTitleBaseline;
        } else {
          lastAuthorBaseline = bodyLines.length > 0 ? bodyFirstBaseline + (bodyLines.length - 1) * BODY_LINE_HEIGHT : bodyFirstBaseline;
        }
        var footerBaseline = lastAuthorBaseline + GAP_AUTHOR_FOOTER;
        var totalH = Math.ceil(footerBaseline + FOOTER_BOTTOM_PAD + PAD_BOTTOM);
        ...
        ctx.font = DAY_FONT;
        ctx.fillText(dateParts.day, CARD_W / 2, dayBaseline);
        
        ctx.font = MONTH_FONT;
        ctx.fillText(dateParts.monthYear, CARD_W / 2, monthBaseline);
        
        ctx.font = WEEK_FONT;
        ctx.fillStyle = COLORS.weekday;
        ctx.fillText(dateParts.week, CARD_W / 2, weekBaseline);
        
        ctx.strokeStyle = COLORS.divider;
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(CARD_W / 2 - DIVIDER_HALF_W, dividerY);
        ctx.lineTo(CARD_W / 2 + DIVIDER_HALF_W, dividerY);
        ctx.stroke();
        ...
        const targetTextWidth = CARD_W - textMarginLeft * 2;
        
        var drawY = bodyFirstBaseline;
        for (let i = 0; i < bodyLines.length; i++) {
          if (i > 0) {
            drawY += BODY_LINE_HEIGHT;
          }
          const line = bodyLines[i];
          const isLastInParagraph = i === bodyLines.length - 1 || bodyLines[i + 1].length < CHARS_PER_LINE;
          if (!isLastInParagraph && line.length > 1) {
            const words = Array.from(line);
            const totalWidth = ctx.measureText(line).width;
            const extraSpace = targetTextWidth - totalWidth;
            const spacePerWord = extraSpace / (words.length - 1);
            let currentX = textMarginLeft;
            for (let j = 0; j < words.length; j++) {
              ctx.fillText(words[j], currentX, drawY);
              currentX += ctx.measureText(words[j]).width + spacePerWord;
            }
          } else {
            ctx.fillText(line, textMarginLeft, drawY);
          }
        }
        ...
        ctx.fillStyle = COLORS.weekday;
        ctx.font = TITLE_FONT;
        drawY = titleFirstBaseline;
        for (let ti = 0; ti < titleLines.length; ti++) {
          if (ti > 0) {
            drawY += TITLE_LINE_STEP;
          }
          ctx.fillText(titleLines[ti], CARD_W / 2, drawY);
        }
        
        ctx.font = AUTHOR_FONT;
        if (authorLines.length > 0) {
          drawY = firstAuthorBaseline;
          for (let ai = 0; ai < authorLines.length; ai++) {
            if (ai > 0) {
              drawY += AUTHOR_LINE_STEP;
            }
            ctx.fillText(authorLines[ai], CARD_W / 2, drawY);
          }
        }
        
        ctx.fillStyle = COLORS.watermark;
        ctx.font = FOOTER_FONT;
        ctx.fillText('微信读书', CARD_W / 2, footerBaseline);
...
let noteRoamingViewInstance: NoteRoamingView | undefined;

export function initializeNoteRoamingView(extensionUri: vscode.Uri): NoteRoamingView {
  noteRoamingViewInstance = new NoteRoamingView(extensionUri);
  return noteRoamingViewInstance;
}

export function getNoteRoamingView(): NoteRoamingView {
